...
export default async function IngredientPage({ params }: Params) {
  ...
  const s = t.ingredients.sections;

  const sections: { title: string; text: string | null }[] = [
    { title: s.scientificName, text: ingredient.scientific_name },
    { title: s.origin, text: ingredient.origin },
    { title: s.properties, text: ingredient.properties },
    { title: s.applications, text: ingredient.applications },
  ];

  return (
    <article className="container-brand pt-8 md:pt-12">
      <nav aria-label="caminho" className="text-[0.85rem] text-ink/70">
        <Link href="/ingredientes" className="hover:underline underline-offset-4">
          {t.nav.ingredients}
        </Link>
      ...
      <header className="mt-6 max-w-3xl">
        <Label tone="violet">{ingredient.category}</Label>
        <h1 className="mt-3 text-h1 text-forest lowercase">{ingredient.name}</h1>
      ...
      <div className="mt-12 grid gap-10 md:grid-cols-12">
        ...
        <aside className="md:col-span-4 md:col-start-9">
          <div className="card-brand p-6">
            <p className="label-brand text-moss">{t.ingredients.productsUsing}</p>
            {products.length ? (
              ...
              <p className="mt-4 text-[0.92rem] text-ink/70">{t.ingredients.noProducts}</p>
            )}
          </div>
          <Link href="/ingredientes" className="mt-6 inline-flex min-h-11 items-center font-ui text-[0.92rem] font-medium text-moss hover:underline underline-offset-4">
            {t.ingredients.backToIndex}
          </Link>
        </aside>
      </div>
